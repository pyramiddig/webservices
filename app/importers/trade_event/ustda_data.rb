...
module TradeEvent
  class UstdaData
    ...
    def setup
      @loaded_resource = open(@resource, 'r:UTF-8').read.encode('UTF-16le', invalid: :replace, replace: '', universal_newline: true).encode('UTF-8')
    end
    ...
    def events
      doc = CSV.parse(@loaded_resource, headers: true, header_converters: :symbol, encoding: 'UTF-8', skip_lines: /^"Last updated/)
